...
const TableProduct = ({cart}) => {
  ...
  <tbody>
    {cart?.items?.map((product)=>
        ...
        onClick={() => addToCart(product?.productId, product.title, product.price / product.qty, 1, product.imgsrc)} style={{cursor:"pointer"}}>
add_circle
</span></td>
        <td className='bg-dark text-light'><span className="material-symbols-outlined"
        onClick={() => decreaseQty(product?.productId, 1)} style={{cursor:"pointer"}}>
do_not_disturb_on
</span></td>
        <td className='bg-dark text-light'> <span className="material-symbols-outlined" style={{cursor:"pointer"}}
            onClick={() => {
